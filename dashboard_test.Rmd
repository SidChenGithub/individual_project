...
#create a new dataframe to include all values we have
...
#calculate some statistical attributes of portfolio performance
...
alpha_output <- paste0(percent(myportfolio_model[1,]$estimate,
                               accuracy = 0.001),
                      " (",
                      round(myportfolio_model[1,]$statistic, 3),
                      ")")

beta_output <- paste0(round(myportfolio_model[2,]$estimate, 3),
                      " (",
                      round(myportfolio_model[2,]$statistic, 3),
                      ")")

#calculate excess return over risk-free rate of the portfolio and SP500
myportfolio_condensed <- myportfolio_condensed %>% 
  mutate(portfolio_excess_return = portfolio_return - tbill_yield_monthly$t_bill_yield,
         SP500_excess_return = SPY_return - tbill_yield_monthly$t_bill_yield)
...
fig1 <- ggplot(myportfolio_relative_returns,
  ...
  ggplotly(fig1, tooltip = c("x", "text"))
...
-----------------------------------------------------------------------
### Component Stocks' Contributions to Portfolio's Total Relative Return

```{r}

#add text= in aes() to set tooltip for ggplotly
fig2 <- ggplot(myportfolio_value,
...
Row 3 {data-height=140}
----------------------------------
### Summary of the portfolio performance (all in monthly basis)

```{r}

#consolidate every data we need in the table
fig3<-data.frame(matrix(nrow = 1,
...
colnames(fig3) <- c("Average return",
                    "Volatility",
                    "Corr. with SP500",
                    "Alpha (t-statistic)",
                    "Beta (t-statistic)",
